...
export function loginPage(demoMode: boolean, onAuthenticated: () => Promise<void>): HTMLElement {
  ...
  root.innerHTML = `<section class="auth-brand"><img src="/brand/moneda-logo.svg" alt="Moneda Technologies"><div class="auth-message"><span class="eyebrow">Business, clearly managed</span><h1>From product detail<br>to commercial decision.</h1><p>One governed workspace for EUR pricing, quotations, customers and the sales pipeline.</p><div class="auth-points"><span><i data-lucide="shield-check"></i>Server-authoritative pricing</span><span><i data-lucide="refresh-cw"></i>Live EUR conversion</span><span><i data-lucide="file-lock-2"></i>Immutable quotation history</span></div></div><div class="auth-stripe"><span></span><span></span><span></span></div></section><section class="auth-form-panel"><div class="auth-form-wrap"><div class="mobile-auth-logo"><img src="/brand/moneda-logo.svg" alt="Moneda Technologies"></div><span class="eyebrow">Welcome to Moneda</span><h2>Sign in to your workspace</h2><p>Use your account credentials or a secure email verification code.</p><div class="auth-methods" role="tablist"><button class="active" data-auth-method="password" type="button"><i data-lucide="key-round"></i>ID / password</button><button data-auth-method="otp" type="button"><i data-lucide="mail-check"></i>Email OTP</button></div><form id="password-form" class="auth-form"><label>ID / username<div class="input-icon"><i data-lucide="user-round"></i><input name="identifier" required autocomplete="username" placeholder="Admin"></div></label><label>Password<div class="input-icon"><i data-lucide="lock-keyhole"></i><input name="password" type="password" required autocomplete="current-password" placeholder="Enter your password"></div></label><div class="auth-form-links"><a href="/forgot-password">Forgot password?</a></div><button class="button button-primary button-full" type="submit">Sign in<i data-lucide="arrow-right"></i></button></form><form id="email-form" class="auth-form hidden"><label>Email address<div class="input-icon"><i data-lucide="mail"></i><input name="email" type="email" required autocomplete="email" placeholder="you@company.com"></div></label><button class="button button-primary button-full" type="submit">Send secure code<i data-lucide="arrow-right"></i></button></form><form id="otp-form" class="auth-form hidden"><button class="back-link" type="button"><i data-lucide="arrow-left"></i>Use another email</button><label>Verification code<input name="code" class="otp-input" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" required autocomplete="one-time-code" placeholder="000000"></label><button class="button button-primary button-full" type="submit">Verify and sign in<i data-lucide="arrow-right"></i></button></form>${demoMode ? '<div class="auth-divider"><span>local demo access</span></div><button id="demo-login" class="button button-demo button-full"><i data-lucide="sparkles"></i>Enter demo workspace</button><p class="demo-copy">Superadmin: <strong>Admin</strong> / <strong>123@Admin</strong></p>' : ""}<p class="auth-legal">New to Moneda? <a href="/signup">Create an account</a></p></div><footer><span>© ${new Date().getFullYear()} Moneda Technologies</span><a href="#">Privacy</a><a href="#">Support</a></footer></section>`;
  root.querySelectorAll<HTMLImageElement>("img").forEach((image) => { image.src = appStore.state.brandLogoPath; image.alt = appStore.state.brandName; });
  ...
  emailForm.addEventListener("submit", async (event) => {
    ...
    catch (error) { toast(error instanceof Error ? error.message : "Verification could not be sent", "error"); }
