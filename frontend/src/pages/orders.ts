...
async function showOrderDetails(order: Record<string, unknown>): Promise<void> {
  ...
  detailActions.querySelector<HTMLButtonElement>("[data-detail-pdf]")?.addEventListener("click", () => openPdfViewer({
    title: "Order Confirmation PDF", subtitle: `${String(order.order_number ?? order._id ?? "Order")} · immutable PDF`,
    filename: `${String(order.order_number ?? order._id ?? "order-confirmation")}.pdf`, path: `/orders/${encodeURIComponent(String(order._id))}/pdf?preview=true`,
  }));
...
export async function orderDetailPage(orderId: string): Promise<HTMLElement> {
  ...
  if (changesSection) body.querySelector("#audit-history")?.insertAdjacentHTML("beforebegin", changesSection);
  if (revisions.length) {
    const markup = `<section class="panel revision-history"><div class="section-title"><div><span class="eyebrow">Revision History</span><h2>${escapeHtml(String(order.order_number ?? order._id))}</h2></div><span class="count-badge">Current V${escapeHtml(String(order.version ?? 1))}</span></div><div class="timeline-list">${revisions.map((version) => `<div class="timeline-item"><i data-lucide="history"></i><div><strong>V${escapeHtml(String(version.version))}${Number(version.version) === Number(order.version ?? 1) ? " · Current" : ""}</strong><small>${escapeHtml(String((version.created_by_snapshot as Record<string, unknown> | undefined)?.name ?? version.created_by ?? "User"))} · ${escapeHtml(String(version.reason ?? "Revision"))}</small><button class="button button-quiet button-small" type="button" data-order-version-pdf="${escapeHtml(String(version._id))}" data-version="${escapeHtml(String(version.version))}">View PDF</button></div></div>`).join("")}</div></section>`;
    body.querySelector(".detail-layout")?.insertAdjacentHTML("beforeend", markup);
  }
