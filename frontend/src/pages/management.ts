...
export async function usersPage(): Promise<HTMLElement> {
  ...
  const editUser = (user: Record<string, unknown>, roles: Record<string, unknown>[], customers: Record<string, unknown>[], managers: Record<string, unknown>[], reload: () => Promise<void>) => {
    ...
    const syncIncentiveField = () => {
     ...
     syncIncentiveField();
      renderAssignments();
     content.querySelector<HTMLButtonElement>("[data-set-user-password]")?.addEventListener("click", () => {
       const passwordContent = document.createElement("div");
       passwordContent.innerHTML = `<form class="stack-form"><p class="form-hint">Set a password for ${escapeHtml(String(user.name ?? "this user"))}. Use at least 8 characters with one uppercase letter, one lowercase letter and one number.</p><label>New password<input name="password" type="password" minlength="8" pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{8,}" autocomplete="new-password" required></label><label>Confirm password<input name="confirm_password" type="password" minlength="8" autocomplete="new-password" required></label><small class="field-error" data-password-error></small><button class="button button-primary button-full" type="submit">Update password</button></form>`;
       const passwordDialog = openModal("Set user password", passwordContent);
       enhancePasswordFields(passwordContent);
       passwordContent.querySelector("form")?.addEventListener("submit", async (event) => {
         event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const data = new FormData(form); const password = String(data.get("password") ?? ""); const confirm = String(data.get("confirm_password") ?? ""); const error = passwordContent.querySelector<HTMLElement>("[data-password-error]");
         ...
         try { await adminApi.setUserPassword(String(user._id), password, confirm); passwordDialog.close(); toast("Password updated"); } catch (err) { if (error) error.textContent = err instanceof Error ? err.message : "Password could not be updated"; button.disabled = false; }
       });
       refreshIcons(passwordContent);
     });
  ...
  try {
    ...
    if (appStore.can("settings.view")) {
      ...
      try {
        ...
        const state = status.configuration_error === "WORKDRIVE_AUTHORIZATION_REQUIRED" ? "WorkDrive authorization required" : status.configuration_error === "WORKDRIVE_ROOT_FOLDER_REQUIRED" ? "Root folder ID required" : status.configuration_error === "WORKDRIVE_AUTHORIZATION_INVALID" ? "WorkDrive authorization expired/invalid" : status.configuration_error ? "WorkDrive configuration requires attention" : status.connected ? "Connected" : status.last_test_status === "error" ? "Connection error" : "Not connected";
        const syncState = status.sync.status === "synced" ? "Synced" : status.sync.status === "failed" ? "Failed" : status.sync.missing_local ? "Needs attention" : status.sync.pending ? "Pending" : "No local assets";
    ...
    page.querySelector<HTMLButtonElement>("[data-workdrive-test]")?.addEventListener("click", async (event) => {
      ...
      try { await adminApi.workdriveTest(); if (result) result.innerHTML = '<div class="notice compact"><i data-lucide="circle-check"></i><div><strong>WorkDrive connection is healthy</strong></div></div>'; toast("WorkDrive connection is healthy"); refreshIcons(result ?? page); }
        ...
        const message = error instanceof Error ? error.message : "WorkDrive test failed";
        const reference = error instanceof ApiError && error.reference ? `<p>Reference ${escapeHtml(error.reference)}</p>` : "";
        if (result) {
          result.innerHTML = `<div class="notice error compact"><i data-lucide="circle-alert"></i><div><strong>${escapeHtml(message)}</strong>${reference}</div></div>`;
          refreshIcons(result);
        }
        toast(message, "error");
      }
