...
export async function customerDetailPage(customerId: string): Promise<HTMLElement> {
  ...
  const load = async () => {
    ...
    const detailFields = [...body.querySelectorAll<HTMLElement>(".detail-grid > div")];
    detailFields.find((field) => field.querySelector("span")?.textContent === "Tax / GST number")?.remove();
    const currencyField = detailFields.find((field) => field.querySelector("span")?.textContent === "Currency");
    if (currencyField?.querySelector("span")) currencyField.querySelector("span")!.textContent = "Display currency";
