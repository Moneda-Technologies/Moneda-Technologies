...
async function openOrderConversionModal(quotationId: string, onConverted: () => Promise<void>): Promise<void> {
  ...
  const addRecipient = () => {
    if (recipientList.children.length >= 5) return;
    const row = document.createElement("div");
    row.className = "oc-recipient-row";
    row.innerHTML = `<input name="additional_recipient" type="email" placeholder="email@example.com" required><button class="button button-quiet" type="button" data-remove-recipient aria-label="Remove recipient"><i data-lucide="x"></i></button>`;
    recipientList.appendChild(row);
    refreshIcons(row);
  };
  content.querySelector("[data-cancel]")?.addEventListener("click", () => dialog.close());
  content.querySelector("[data-add-recipients]")?.addEventListener("click", () => { recipientSection.hidden = false; if (!recipientList.children.length) addRecipient(); });
  content.querySelector("[data-add-recipient]")?.addEventListener("click", addRecipient);
  recipientList.addEventListener("click", (event) => {
    const remove = (event.target as HTMLElement).closest("[data-remove-recipient]");
...
export async function quotationsPage(): Promise<HTMLElement> {
  ...
  try {
    ...
    body.addEventListener("click", (event) => {
      ...
      openOrderConversionModal(target.dataset.id!, load).catch((error) => toast(error instanceof Error ? error.message : "Order Confirmation could not be opened", "error")).finally(() => { target.disabled = false; });
    }, true);
