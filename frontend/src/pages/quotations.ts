import { cartApi, customerCompanyApi, quotationApi } from "../api";
import { refreshIcons } from "../components/icons";
import { openModal } from "../components/modal";
import { pageScaffold, statusBadge } from "../components/page";
import { toast } from "../components/toast";
import { appStore } from "../state/store";
import type { CartItem, Quotation } from "../types/domain";
...
function cartLine(item: CartItem, editableOrIndex: boolean | number = true): string {
  ...
  const masterUnitPrice = line.quantity ? masterSubtotal / line.quantity : masterSubtotal;
...
export async function quotationsPage(): Promise<HTMLElement> {
  ...
  try {
    ...
    const load = async () => {
      ...
      const fields: Record<string, string> = { search: get("quotation-search"), customer_id: get("quotation-customer"), status: get("quotation-status"), currency: get("quotation-currency"), region: get("quotation-region"), country: get("quotation-country"), from_date: get("quotation-from"), to_date: get("quotation-to"), min_total: get("quotation-min"), max_total: get("quotation-max") };
    ...
    ["quotation-customer", "quotation-status", "quotation-currency", "quotation-region", "quotation-country", "quotation-from", "quotation-to", "quotation-min", "quotation-max"].forEach((id) => body.querySelector(`#${id}`)?.addEventListener("change", reload));
...
export async function quotationPreparationPage(): Promise<HTMLElement> {
  ...
  try {
    ...
    body.innerHTML = `<div class="workspace-steps"><span class="done"><b>1</b>Customer</span><span class="done"><b>2</b>Products</span><span class="done"><b>3</b>Cart</span><span class="active"><b>4</b>Quotation</span></div><div class="quote-layout"><section class="quote-main"><section class="panel quotation-customer-card"><span class="eyebrow">Customer</span><h2>${escapeHtml(context.name)}</h2><p class="muted">Issued by Moneda Technologies in EUR to the selected customer.</p></section><section class="panel quotation-items-panel"><div class="section-title"><div><span class="eyebrow">EUR Quotation Items</span><h2>Items being prepared</h2></div><span class="count-badge">${cart.items.length} line item${cart.items.length === 1 ? "" : "s"}</span></div><div class="cart-lines">${cart.items.map((item) => cartLine(item, false)).join("")}</div></section></section>${cartTotalsMarkup(cart, "EUR", "")}</div>`;
    ...
    const summary = body.querySelector<HTMLElement>(".quote-summary")!; const form = document.createElement("form"); form.id = "quote-details-form"; form.className = "stack-form"; form.innerHTML = `<label>Quotation Currency<input value="EUR" disabled><small>All Moneda quotations are issued in EUR.</small></label><label>Proforma Validity (days)<input name="proforma_validity_days" type="number" min="1" max="365" value="30" required></label><label>Payment Terms<select name="payment_terms"><option>Advance</option><option>POD</option><option>15 Days</option><option>30 Days</option></select></label><label>Transport<select name="transport_mode"><option value="by_consignee">By Consignee</option><option value="by_moneda_team">By Moneda Team</option></select></label><label class="transport-charge" hidden>Transport Charges (EUR)<input name="transport_charges" type="number" min="0" step="0.01" placeholder="Enter freight charge"></label><label>Customer Notes<textarea name="customer_notes" rows="4" placeholder="Add quotation-specific notes for the customer"></textarea></label><label class="check-row"><input name="create_lead" type="checkbox" checked><span>Create a linked CRM lead and follow-up</span></label><label>Follow-up Date<input name="follow_up_date" type="date"></label><button class="button button-primary button-full"><i data-lucide="eye"></i>Preview Quotation</button>`; summary.append(form);
