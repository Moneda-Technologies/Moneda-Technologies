...
async function openPriceEditor(row: PricingResource, reload: () => Promise<void>): Promise<void> {
  ...
  try {
    ...
    const renderPreview = () => {
      ...
      if (!preview) return;
      if (!(price >= 0) || !rates.rates.USD || !rates.rates.INR) { preview.innerHTML = `<span class="eyebrow">Live equivalents</span><strong>Unavailable</strong><small>Enter a EUR master price to preview conversions.</small>`; return; }
      preview.innerHTML = `<span class="eyebrow">Live equivalents · ${rates.source === "cached" || rates.stale ? "Cached" : "ECB live"}</span><div><strong>USD $${(price * rates.rates.USD).toFixed(2)}</strong><strong>INR ₹${(price * rates.rates.INR).toFixed(2)}</strong></div><small>Read-only conversion from EUR master.</small>`;
    };
    priceInput.addEventListener("input", renderPreview);
    renderPreview();
  } catch { if (preview) preview.innerHTML = `<span class="eyebrow">Live equivalents</span><strong>FX rates unavailable</strong><small>EUR master editing remains available.</small>`; }
