...
export const orderApi = { list: (customerId: string) => api<{ items: Record<string, unknown>[]; total: number }>(`/orders?customer_id=${encodeURIComponent(customerId)}`), get: (id: string) => api<Record<string, unknown>>(`/orders/${encodeURIComponent(id)}`) };
